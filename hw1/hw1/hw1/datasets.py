...
def torch_temporary_seed(seed: int):
    ...
    # Set the new temporary random seed
...
class ImageStreamDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterator[Tuple[Tensor, int]]:
        ...
        # ====== YOUR CODE: ======
        while True:  # Infinite loop to generate images
...
class SubsetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if adjusted_index >= len(self.source_dataset):  # Check against the length of the source dataset
            raise IndexError("Index out of bounds in the source dataset")
